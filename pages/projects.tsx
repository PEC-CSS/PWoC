...
const Projects: NextPage = () => {
	...
	return (
		<PageLayout title='PWOC | Projects' description='List of the available projects in PWoC. Choose and contribute.'>
			<div>
				<Search onSearch={(e) => { e.preventDefault(); }} searchTerm={searchTerm} setSearchTerm={setSearchTerm} />
				{!projects ? (
					isClient && <Lottie play loop animationData={loading} className='h-[200px] w-auto' />
				) : projectSearchResults.length > 0 ? (
					<SearchResults projects={projectSearchResults} />
				) : (
					<div className='flex flex-col items-center mb-[20px]'>
						{isClient && <Lottie play loop animationData={notfound} className='h-[300px] w-auto' />}
